...
__host__ void generate_rand_data(unsigned int * data, unsigned int num_elements)
{
	...
	{
		data[i] = static_cast<unsigned int>(rand() % 10);
	}
}
				...
				const unsigned int tid)
{
	...
	const unsigned int val = data[tid];
	const unsigned int square = val * val;
	data[tid] = square;
}
...
__host__ float execute_register_memory_operations(void)
{
	...
	//console print the host data after the GPU kernel
	printf("REGISTER:\n");
	...
	cudaFree((void*)d_data);  //free device data
