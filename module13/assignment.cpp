...
void waitForTimingEvent(cl_event* event, const char* descriptor)
{
...
int ExecuteKernel(cl_event* event, const cl_program program, const cl_command_queue commandQueue, const char* kernelName, const cl_mem* memObjects)
{
...
{
    int c;
    while ((c = getopt(argc, argv, "ebmn:")) != -1)
    {
        switch (c)
        {
        case 'e':
    ...
    if (eflag + bflag + mflag > 1)
    {
        printf("Only one flag allowed at a time");
        exit(1);
    }
    ...
    for (int i = 0; i < iters; ++i)
    {
        ...
        const char* kernelNames[numKernels]{ "add", "sub", "mult", "div", "power" };
        cl_event events[numKernels];
        int result = 0;
        for (int i = 0; i < numKernels; ++i)
        {
            result += ExecuteKernel(&events[i], program, commandQueue, kernelNames[i], memObjects);
        }
        ...
        {
            cl_event event;
            clEnqueueMarker(commandQueue, &event);
            waitForTimingEvent(&event, "time between last kernel queued and all kernels done");
        }
        ...
        if (eflag && result == 0)
        {
            for (int i = 0; i < numKernels; ++i)
            {
                waitForTimingEvent(&events[i], kernelNames[i]);
            }
        }

        if (result != 0)
        {
            printf("Error executing kernels\n");
        }
